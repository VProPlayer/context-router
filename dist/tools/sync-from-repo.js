...
async function syncRepo(repo) {
    ...
    if (commits.length === 0)
        return `_No commits found in ${owner}/${name}@${branch}_`;
    ...
    if (newCommits.length === 0)
        return `_Already up to date (last synced: ${lastSyncedCommit})_`;
...
export async function syncFromRepo(config, projectKey) {
    ...
    if (project.repos.length === 0)
        throw new Error(`No repos configured for project: ${projectKey}`);
    const results = await Promise.all(project.repos.map(async (repo) => {
