...
export async function writeProject(config: Config, projectKey: string, content: string): Promise<string> {
  ...
  if (!project) throw new Error(`Unknown project: "${projectKey}". Run list_projects to see available keys.`);
  if (!project.writeBack) throw new Error(`Write-back is disabled for project: ${projectKey}`);
